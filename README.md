...
2. 打刻機能  
   ...
   - 日をまたぐ勤務を許容する（例：22:00 出勤 → 翌02:00 退勤）  
   - 勤務日は**出勤打刻時の日付（JSTベース）**を基準とする  
   - 退勤打刻時刻が出勤時刻より小さい場合は自動的に翌日分として `clock_out` の日付を補正する  

3. 勤務時間計算機能  
   - 複数回の休憩時間をすべて合算し、実働時間を自動算出  
   - 日別詳細画面で各休憩の開始・終了を時系列で確認可能  
   - 月別の合計実働時間表示  
   - `clock_in` / `clock_out` はともに `datetime` 型で保持し、日付跨ぎが発生しても差分から正確な実働時間を算出する  
   - 月別集計・CSV出力はいずれも**出勤日（`clock_in` の日付）基準**で集計する  
